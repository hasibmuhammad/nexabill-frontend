...
export function EmployeesFilters({
  ...
  filters: { status: string; designation: string };
  onFilterChange: (
    key: keyof { status: string; designation: string },
    value: string
  ) => void;
  isLoading?: boolean;
}) {
  ...
  const activeCount = useMemo(() => {
    let c = 0;
    if (local.status) c += 1;
    if (local.designation) c += 1;
    return c;
  }, [local.status, local.designation]);
  ...
  return (
    <div className="relative flex items-center space-x-3">
      ...
      {showFilters && (
        <div
          ref={filterRef}
          className="absolute top-full left-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg z-40 p-6 min-w-[520px]"
        >
          ...
          <div className="flex items-center justify-end space-x-3 mt-6 pt-4 border-t border-slate-200 dark:border-slate-700">
            ...
            <Button
              ...
              size="sm"
              onClick={() => {
                onFilterChange("status", local.status);
                onFilterChange("designation", local.designation);
