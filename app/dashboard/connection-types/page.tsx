...
import { connectionTypeSchema, type ConnectionTypeFormValues } from "@/lib/schemas/connection-type";
...
function ConnectionTypeForm({
  ...
  onCancel: () => void;
  onSubmit: (values: ConnectionTypeFormValues) => void;
  submitting?: boolean;
}) {
  const {
    register,
    ...
    reset,
    formState: { errors },
  } = useForm<ConnectionTypeFormValues>({
    resolver: zodResolver(connectionTypeSchema),
    defaultValues: {
      name: initial?.name || "",
