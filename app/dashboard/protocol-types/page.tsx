...
export default function ProtocolTypesPage() {
  ...
  const { data: protocolTypes, isLoading } = useQuery({
    queryKey: ["protocolTypes"],
    queryFn: async () => {
      // TODO: Replace with actual API call
      // Generate mock data for testing
      const mockProtocolTypes: ProtocolType[] = Array.from(
        { length: 80 },
        (_, i) => ({
          id: `protocol-${i + 1}`,
          name: `Protocol Type ${i + 1}`,
          description: `Description for Protocol Type ${i + 1}`,
  ...
  const createMut = useMutation({
    mutationFn: async (data: {
      ...
      // TODO: Replace with actual API call
      console.log("Creating protocol type:", data);
      return {
        id: "temp-" + Date.now(),
        ...data,
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      toast.success("Protocol type created successfully");
    },
    onError: () => {
      toast.error("Failed to create protocol type");
    },
  });
  ...
  const updateMut = useMutation({
    mutationFn: async ({
      ...
      // TODO: Replace with actual API call
      console.log("Updating protocol type:", id, data);
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      setEditing(null);
      toast.success("Protocol type updated successfully");
    },
    onError: () => {
      toast.error("Failed to update protocol type");
    },
  });
  ...
  const deleteMut = useMutation({
    mutationFn: async (id: string) => {
      // TODO: Replace with actual API call
      console.log("Deleting protocol type:", id);
      return { success: true };
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["protocolTypes"] });
      toast.success("Protocol type deleted successfully");
    },
    onError: () => {
      toast.error("Failed to delete protocol type");
    },
  });

  const startEdit = (protocolType: ProtocolType) => {
    setEditing(protocolType);
    setShowForm(true);
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 theme-transition">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex space-x-3">
              ...
              >
                <Plus className="h-4 w-4 mr-2" /> New Protocol Type
              </Button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {isLoading ? (
          <PageLoader message="Loading protocol types..." />
        ) : (
          <>
            <Card className="p-6 mb-6">
              <div className="flex items-center justify-between">
                <div className="relative flex-1 max-w-md">
                  ...
                  <Input
                    placeholder="Search protocol types by name or description..."
                    value={search}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                  ...
                  <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                    {paginatedData.map((protocolType, index) => (
                      <tr
                        key={protocolType.id}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          >
                            {protocolType.isActive ? "Active" : "Inactive"}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700 dark:text-slate-300">
                          {new Date(
                            protocolType.createdAt
                          ).toLocaleDateString()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => startEdit(protocolType)}
                            >
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (
                                  confirm(
                                    "Are you sure you want to delete this protocol type?"
                                  )
                                ) {
                                  deleteMut.mutate(protocolType.id);
                                }
                              }}
...
function ProtocolTypeForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      toast.error("Protocol type name is required");
      return;
    }
    onSubmit({
      name: name.trim(),
    ...
    <form id="protocol-type-form" onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          placeholder="Enter protocol type name"
          required
        />
      </div>
